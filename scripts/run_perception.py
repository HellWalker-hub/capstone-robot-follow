"""
Laptop prototype — run full perception pipeline on webcam.
Click on a person in the first frame to register as target.
...
def find_bbox_at_click(tracks, point):
    """Return bbox of track containing click point."""
    px, py = point
    for track in tracks:
...
def draw_overlay(frame, result):
    ...
    for track in result["all_tracks"]:
        ...
        c = (0, 255, 0) if is_target else (200, 200, 200)
        thickness = 3 if is_target else 1
        cv2.rectangle(frame, (x1, y1), (x2, y2), c, thickness)
        cv2.putText(frame, f"ID:{tid}", (x1, y1 - 5),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, c, 1)

    label = f"State: {state.value.upper()}"
    cv2.rectangle(frame, (0, 0), (260, 30), (0, 0, 0), -1)
    cv2.putText(frame, label, (5, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)

    if state == RPFState.IDLE:
        cv2.putText(frame, "Click person to follow", (5, 50),
...
def main():
    ...
    print("Click on a person to register as follow target. Press 'q' to quit, 'r' to reset.")
    ...
    while True:
        ...
        # handle mouse click for target registration
        if clicked_point is not None:
            bbox = find_bbox_at_click(result["all_tracks"], clicked_point)
            if bbox is not None:
                pipeline.register_target(frame, bbox)
                print(f"[Main] Registered target at bbox {bbox}")
            else:
                print("[Main] No person at click location.")
            clicked_point = None
